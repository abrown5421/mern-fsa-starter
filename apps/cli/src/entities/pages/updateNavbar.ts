...
export async function updateNavbar(pageName: string, routePath: string) {
  ...
  let navContent = fs.readFileSync(navbarFile, 'utf-8');
    ...
    const formatted = await prettier.format(navContent, { parser: 'typescript' });
  ...
  let drawerContent = fs.readFileSync(drawerFile, 'utf-8');
    ...
    const formattedDrawer = await prettier.format(drawerContent, { parser: 'typescript' });
